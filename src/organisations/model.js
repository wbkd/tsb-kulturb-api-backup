...
module.exports = (mongoose) => {
  ...
  const Organisation = new Schema(
    {
      ...
      types: [
        {
          type: String,
          enum: ['organisation', 'venue', 'organisation and venue'],
        },
      ],
    },
  ...
  Organisation.pre('save', async function importOSMData(next) {
    try {
      const { accessibilityWheelchair, accessibilityBlind, accessibilityDeaf, openingHours } = await osm.getOSMData(
        this
      );
